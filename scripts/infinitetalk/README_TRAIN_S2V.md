# InfiniteTalk-S2V Full Parameter Training Guide

This document provides a complete workflow for full parameter training of InfiniteTalk-S2V (audio-driven long-form talking head video generation model), including environment configuration, data preparation, distributed training, and inference testing.

> **Note**: InfiniteTalk is an audio-driven long-form talking head video generation model that requires both a reference image and an audio file to generate talking videos. The training data needs to include video, audio, and reference images.
...
> ⚠️ **Important**: InfiniteTalk-S2V is an audio-driven long-form talking head video generation model. Unlike regular video training, you **must provide the `audio_path` field in metadata.json**.
...
- `audio_path`: Audio file path (**InfiniteTalk-S2V specific and required**, main difference from regular video training)
...
**Dataset Comparison: InfiniteTalk-S2V vs Regular Video Training**:

| Model Type | Required Fields | Audio Field |
|---------|---------|---------|
| Regular Video (WAN, CogVideoX, etc.) | `file_path`, `text`, `type` | ❌ Not needed |
| **InfiniteTalk-S2V (Audio-Driven Generation)** | `file_path`, `audio_path`, `type` | ✅ **Required** |
...
| `--motion_frames` | Motion frames for InfiniteTalk long-form generation | 9 |
...
| `--seed` | Random seed | 43 |
| `--output_dir` | Output directory | `output_dir_infinitetalk` |
...
| `--transformer_path` | Pretrained Transformer weights path | `models/Personalized_Model/infinitetalk.safetensors` |
| `--trainable_modules` | Trainable modules (`"."` means all modules, `"audio"` means audio modules only) | `"audio"` |
...
| `config_path` | Model configuration file path | `config/wan2.1/wan_civitai.yaml` |
| `model_name` | Model path | `models/Diffusion_Transformer/Wan2.1-I2V-14B-480P` |
| `model_name_audio` | Audio encoder path | `models/Diffusion_Transformer/chinese-wav2vec2-base/` |
| `sampler_name` | Sampler type: `Flow`, `Flow_Unipc`, `Flow_DPM++` | `Flow` |
| `shift` | Sampler shift parameter | 5.0 |
| `transformer_path` | Path to trained Transformer weights | `models/Personalized_Model/infinitetalk.safetensors` |
...
| `segment_frame_length` | Number of frames per segment | 81 |
...
| `ref_image` | Reference image path | `"asset/8.png"` |
| `audio_path` | Input audio path | `"asset/talk.wav"` |
| `prompt` | Generation prompt | `"一个人在说话。"` |
| `negative_prompt` | Negative prompt | See code |
| `guidance_scale` | Prompt guidance strength | 5.0 |
...
| `save_path` | Generated video save path | `samples/infitetalk-videos` |
| `max_frames_num` | Total frames to generate | 500 |
| `color_correction_strength` | Color correction strength (0.0-1.0) | 1 |
| `use_apg` | Use Adaptive Projected Guidance | `False` |
...
| `teacache_threshold` | TeaCache threshold (recommended 0.05~0.30) | `0.20` |
...
Edit `examples/infinitetalk/predict_s2v.py` according to your needs. For first-time inference, focus on these parameters. For other parameters, see the Inference Parameters section above.
...
# Trained weights path, e.g. "output_dir_infinitetalk/checkpoint-xxx/diffusion_pytorch_model.safetensors"
transformer_path = "models/Personalized_Model/infinitetalk.safetensors"
...
Edit `examples/infinitetalk/predict_s2v.py`:
