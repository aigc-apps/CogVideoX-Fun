# MiniMax-H3 LoRA 训练指南

本文档提供 MiniMax-H3 LoRA（低秩适配）训练的完整工作流，包括环境配置、数据准备、分布式训练和推理测试。
...
推荐使用 FSDP 训练 MiniMax-H3。即使使用 LoRA，冻结的 transformer（bfloat16 下约 62 GB）和 Qwen3-VL 条件器（约 62 GB）仍需在多卡间分片——FSDP（`FULL_SHARD`）可以分片权重，而 DeepSpeed-Zero-2 不能。
...
| `--target_name` | 应用 LoRA 的组件/模块（逗号分隔） | `to_q,to_k,to_v,ff.0,ff.2,proj_in,audio_proj_in,context_embedder` |
| `--use_peft_lora` | 使用 PEFT 模块添加 LoRA（更省显存，可选） | - |
...
| `--learning_rate` | 初始学习率（LoRA 通常使用比全参训练更高的学习率） | 1e-4 |
| `--seed` | 随机种子 | 42 |
| `--output_dir` | 输出目录 | `output_dir_minimax_h3_lora` |
...
| `lora_path` | LoRA 权重路径 | `output_dir_minimax_h3_lora/checkpoint-xxx/lora_diffusion_pytorch_model.safetensors` |
...
按需编辑 `examples/minimax_h3/predict_t2v.py`。LoRA 推理请重点关注以下参数：
...
# LoRA 权重路径，如 "output_dir_minimax_h3_lora/checkpoint-xxx/lora_diffusion_pytorch_model.safetensors"
lora_path = None
# LoRA 权重强度
lora_weight = 0.55  
# 按要生成的内容填写
